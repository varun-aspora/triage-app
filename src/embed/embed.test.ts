...
import { HASH_MODEL, createEmbedder, type EmbedConfig } from './index.ts';
import { createOllamaClient, ollamaEmbedUrl } from './ollama.ts';
import { OPENAI_EMBEDDINGS_URL, createOpenAiClient } from './openai.ts';
...
describe('ollama client', () => {
  test('POSTs {model, input[]} to OLLAMA_BASE_URL/api/embed and parses vectors', async () => {
    ...
    expect(await client(['a', 'b'])).toEqual([
      [1, 2, 3],
      [4, 5, 6],
    ]);
...
describe('openai client', () => {
  test('sends the Authorization header and {model, input[]} to /v1/embeddings', async () => {
    ...
    expect(await client(['first', 'second'])).toEqual([
      [0.1, 0.2],
      [0.3, 0.4],
    ]);
    const { url, init } = fetch.calls[0]!;
