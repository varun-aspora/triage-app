...
describe(".env loading", () => {
  test("bunfig.toml turns off automatic .env loading", () => {
    expect(readFileSync(join(REPO, "bunfig.toml"), "utf8")).toMatch(
      /^env\s*=\s*false\s*$/m,
    );
  });
  ...
  test("a .env in the cwd is not loaded under bun run test", () => {
    ...
    const guarded = runBun(
      tempProject({ ...files, "bunfig.toml": repoBunfig() }),
      ["run", "test"],
    );
...
describe("bun run test file selection", () => {
  test("the test script is bun test over src, test, scripts and integrations", () => {
    expect(pkg.scripts.test).toBe(
      "bun test ./src ./test ./scripts ./integrations",
    );
  });
