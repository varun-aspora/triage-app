---
name: atspl-comms
description: ATSPL comms service (SMS, email and push delivery through vendors). Use when a ATSPL user did not get an SMS, OTP, email or push notification. A stub, with the log name unverified.
metadata:
  kind: service
  entity: atspl
  service: comms
...
The same codebase also runs as `ssfb:comms` and `rtl:comms`, each with its own database.
