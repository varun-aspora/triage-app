---
name: rtl-canopy
description: RTL canopy service (support touchpoints, support chat, in-app FAQs and consent). Use when a RTL issue involves the support chat, an FAQ page or a consent decision. A stub, with the tables and log name unverified.
metadata:
  kind: service
  entity: rtl
  service: canopy
...
The same codebase also runs for ATSPL as `atspl:canopy`, with its own database.
