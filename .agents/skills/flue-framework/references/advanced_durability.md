...
- **`timeoutMs` enforcement is itself disputed across the official docs.** The durability guide (this page's source) says the coordinator fires the attempt's abort signal *preemptively* at the deadline. The Agent API reference describes the same deadline as checked *cooperatively* — before each turn and before recovery work, not during an in-flight provider call — and says a hung provider call can outlive it, falling to the attempt/error budget instead. Don't assume a long-running tool is killed the instant `timeoutMs` elapses; verify against the installed package's behavior for anything timing-sensitive, and prefer `signal`-aware code in tools regardless, since that's what unwinds work under either reading.
