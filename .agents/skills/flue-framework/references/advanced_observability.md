...
| `submission_queued`, `submission_running` | Queue lifecycle before settlement: admitted, then an attempt starts processing. At-least-once — a redelivery or a recovery re-attempt re-emits these for the same `submissionId`. |
| `submission_settled` | A durable submission reached completed, failed, or aborted — the reliable terminal signal. |
| `submission_recovery` | A coordinator recovery/reconciliation step failed or deferred *without* terminalizing the submission — the alerting companion to `submission_settled` for retry loops that never reach a terminal outcome. Re-emitted on every failed wake (~30s) while the condition persists. |
...

{
  ...
  submissionId?: string; // absent for pass-wide failures (operation: 'reconcile_pass')
  kind?: 'dispatch' | 'direct';
  operation: 'materialize_submission' | 'finalize_settlement' | 'reconcile_submission'
    | 'start_submission' | 'process_submission' | 'reconcile_pass' | 'enforce_deadline';
...
- Alerting on submissions that are stuck retrying without ever terminalizing (`submission_recovery`) — `submission_settled` alone misses these.
...
- Deriving per-instance busy/idle state for dashboards or backpressure, without any durable observer state (see pattern below).

## Patterns

**Alerting on stuck retries, not just terminal failures.** `submission_settled` is the reliable terminal signal, but a submission whose recovery keeps failing (an unavailable agent, a persistently failing classification render) never reaches it until it exhausts its `durability.timeoutMs` budget. `submission_recovery` is the companion signal — it re-emits on every failed wake (roughly every 30s) while the condition persists, so alert on recurrence and dedupe by (`submissionId`, `operation`):

```ts
import { observe } from '@flue/runtime';

observe((event) => {
  if (event.type !== 'submission_recovery') return;
  console.error('submission recovery stalled', {
...
**Deriving instance busy/idle with no durable state.** Mark an instance busy on `submission_queued` or `submission_running`, and clear that submission on `submission_settled`, keyed by `submissionId`. Because recovery re-emits `submission_running` on every replacement attempt (and `submission_queued` re-fires on admission replays), the derived set self-heals across a restart or Durable Object eviction without needing to persist anything:

```ts
import { observe } from '@flue/runtime';

const busy = new Map<string, Set<string>>(); // instanceId -> submissionIds keeping it busy
...
observe((event) => {
  ...
  if (event.type === 'submission_queued' || event.type === 'submission_running') {
    ...
    busy.get(event.instanceId)?.delete(event.submissionId);
  }
});
```
...
- **`submission_settled` alone misses stuck retries.** A submission whose recovery keeps failing (an unavailable agent, a persistently-erroring render) only shows up as `submission_recovery`, re-emitted on every failed wake, until it eventually times out and settles. Pair the two events for complete terminal-failure coverage.
- **`submission_queued`/`submission_running` are at-least-once, like every other event.** A deduplicated `idempotencyKey` replay still re-emits `submission_queued` for the same `submissionId`, and every recovery re-attempt re-emits `submission_running` with an incremented `attemptCount` — expected, and what makes the busy/idle derivation pattern self-healing rather than a bug to filter out.
