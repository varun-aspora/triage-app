...
// `vite preview` (bun run serve:web) serves the built dist on the same port and
// proxy, without watching.
...
export default defineConfig(({ mode }) => {
  ...
  const proxy = Object.fromEntries(PROXIED.map((path) => [path, { target: API }]));
