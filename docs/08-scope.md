...
| Ingress | CLI (`run`, `start`, `wait`, `status`, `ask`, `input`, `post`, `feedback`, `doctor`, `preflight`, `tunnel`, `repos sync`, `fixtures review`, `evals`), HTTP API (polling only, bearer auth, no Slack post by default), Claude Code / Codex skill that drives the CLI | D12, D25, D28, D53 |
