...
flowchart TB
    subgraph Ingress["src/ingress"]
        CLI["cli.ts (bin: triage)<br/>run · start · wait · status · ask · post · feedback<br/>doctor · preflight · tunnel · repos sync"]
...
- **Tools**: `resolve_identity` (deterministic, fixed statements; the orchestrator re-runs it when a new id surfaces mid-run), `note_evidence`, `finish_report` (a `harness: true` tool, see §2). The orchestrator has no free-form entity I/O.
- **Subagents**: for each entity in the request's enabled set: `investigate_<entity>` (tier model) and `investigate_<entity>_deep` (`MODEL_TIER_STRONG`); plus `code_walker`. Names are unique per render, as Flue requires. The enabled set is `TRIAGE_ENTITIES` narrowed by `request.hints.entities`, read from initial data.
...
- **Persistent state**: `plan`, `evidence_index`, `escalation: {triggered, reasons[]}`, `finish_retries`.
- **Lifecycle**:
  - `useAgentStart`: writes `input.json` if absent (first submission) and stamps `run_id` into the tools' closure.
  - `useAgentFinish`: if `ctx.response.toolCalls` has no successful `finish_report`, append `{kind:'signal', type:'triage.finish_required', body:…}` once; on the second miss, throw, which settles the submission `failed` with the evidence folder intact.
...
- **Tools**: `logs_search`, `sql_select`, `http_call`, `note_evidence`. SSFB adds the deterministic ports of today's scripts: `get_account_statement`, `detect_silent_reversals`, `encrypt_lookup_value` and `decrypt_fields` when `SSFB_HARBOR_FIELD_ENC_KEY` is set (D34), and `cbs_call` when `SSFB_CBS_VIA_KUBECTL_ENABLED=true`. A tool whose backing env var is blank is still mounted but answers "not configured for <entity>:<service>" so the investigator can record the gap instead of guessing.
...
| `encrypt_lookup_value` | `investigate_ssfb`, only when `SSFB_HARBOR_FIELD_ENC_KEY` is set | `{value, kind: phone \| email \| cif}` | AES-SIV (deterministic) with the key from env, so the ciphertext can be used as a `$n` param in `sql_select` against encrypted columns such as `customer.external_reference_id` or the phone fields; the key never leaves the tool | ciphertext string |
| `decrypt_fields` | `investigate_ssfb`, same condition | `{values: string[]}` (max 20) | AES-SIV decrypt of column values the investigator already fetched; output passes through the **model-facing** redaction profile (PAN/passport masked, phone visible) and is masked again on persist by `note_evidence`; audit line records count, never plaintext | plaintext strings |
...
triage wait   <run_id> [--timeout <s>] --json
