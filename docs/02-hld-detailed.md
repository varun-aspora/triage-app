...
- **Tools**: `resolve_identity` (deterministic, fixed statements; the orchestrator re-runs it when a new id surfaces mid-run), `note_evidence`, `finish_report` (a `harness: true` tool, see §2) and `ask_requester` (one question at a time to the person who started the run, only when nothing else can unblock it; the run parks in `needs_input` until the answer arrives as a new submission, D52). The orchestrator has no free-form entity I/O.
...
- **Persistent state**: `plan`, `evidence_index`, `escalation: {triggered, reasons[]}`, `finish_retries`, and `id_chain` (the run's chain as the last response left it, so ids added mid-run survive the next submission and a new process, D52).
- **Lifecycle**:
  - `useAgentStart`: writes `input.json` if absent (first submission) and stamps `run_id` into the tools' closure.
  - `useAgentFinish`: if `ctx.response.toolCalls` has no successful `finish_report` and no successful `ask_requester` (which parks the run, D52), append `{kind:'signal', type:'triage.finish_required', body:…}` once; on the second miss, throw, which settles the submission `failed` with the evidence folder intact.
...
triage input  <run_id> ["answer"] [--question q1] [--ids k=v…] [--skip]   # answers the question a run is waiting on (needs_input) and resumes it (D52)
...
- A run parked on a question (`phase: needs_input`, D52) shows `status: running` here with no question, and `ask` is not refused while one is open. The web adapter (the record on `GET`, `POST /triage/:run_id/input`) is not built.
